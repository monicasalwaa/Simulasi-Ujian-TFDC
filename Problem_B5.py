...
def solution_B5():
    ...
    with open('daily-max-temperatures.csv') as csvfile:
        ...
        step = 0
        for row in reader:
            temps.append(row[0])  # YOUR CODE HERE
            time_step.append(row[1])  # YOUR CODE HERE
            step=step + 1

    series = np.array(time_step).astype('float32') # YOUR CODE HERE
    ...
    model=tf.keras.models.Sequential([
        # YOUR CODE HERE.
        tf.keras.layers.Conv1D(64, kernel_size=1, input_shape=[None, 1]),
